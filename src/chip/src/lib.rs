...
impl Chip {
        ...
        where P: Timer + Video + Audio + Input
    {
        ...
        for _ in 0..num_cycle { self.cycle(peripheral) }
    ...
    fn cycle<Peripheral>(&mut self, p: &mut Peripheral)
        where Peripheral: Video + Audio + Input
    {
        ...
        decode! { inst =>
            ...
            "8XY5" => (x, y) {
                let rx = self.regs[x].0 as i8 as i16;
                let ry = self.regs[y].0 as i8 as i16;
                let r = rx - ry;
                self.regs[0xF].0 = if r < 0 { 0 } else { 1 };
                self.regs[x].0 = r as u8;
            }
            "8XYE" => (x, y) {
                self.regs[0xF] = (self.regs[x] & MSB) >> 7;
                self.regs[x] = (self.regs[x] << 1) | ((self.regs[y] & MSB) >> 7);
